...
void Print_com(BYTE Mt, string Stringa, long Value);
void Set_mode(BYTE Mode_control);
long Max_minl(long I);      //    ' keep between -255 to 255
void Poke(int addres, byte * value);   // wlc
...
XMEGA
For the XMEGA you need to use CONFIG XPIN.
==================================================================================================
Action
Configure TIMER0.
Syntax
...
notice Notice that the Help was written with the AT90S2313 and AT90S8515 timers in mind.
...
*/
//Need to design an On timer function that calls the routines
//Need to design an Enable function for interupts and ports
...
string Str_tx_1[20];   //       'print in Main do-loop if Tx-enable = 1
long Sum1;
long Vmax_pos[2];
long Vmax_neg[2];
long Factor_ang[2];
...
bit Hctl_xy;
bit Hctl_sel1;  //selection byte bit 1
bit Hctl_sel2;  //selection byte bit 2
bit Hctl_oe;    //selecion oe hctl data
bit Hctl_rst_1; //reset encoder x
bit Hctl_rst_2; //reset encoder y
BYTE * Hctl_data; 
void main(void)
{
	register byte COMPAREA;
	register byte COMPAREB;
	register byte Pwm1a;
	register byte Pwm1b;
	register byte Pina;
	register byte* M1_pwm;
	register byte* M2_pwm;

	Config Configure;
	Configure.Watchdog = 16;
	Configure.stop = true;
	Configure.Int0 = Falling;
	Configure.Int1 = Falling;


	
	Hctl_xy.value = 0;
	Hctl_sel1.value = 0;     // 'selection first byte msb
	Hctl_sel2.value = 1; // 'selection first byte msb
	Hctl_oe.value = 1;             // 'no output enable
	Hctl_rst_1.value = 1;           // 'no reset
	Hctl_rst_2.value = 1;           //'no reset
	...
	*/
	Init_parameter(1);
	Init_parameter(2);
	Configure_pid(1, 500, 100, 500);//       ''configure Motor 1 - pid initialize
	Configure_pid(2, 500, 100, 500);//       'configure Motor 2 - pid initialize
	// Test1_led = True;
	// Motor_led = True;
	...
	do
	{
		...
		// If Tx_enable = 1 Then       'allow transmission //qlx
		//    Tx_enable = 0
...
'-------------------------------------------------------------------------------
*/
void _declspec (naked) Int_0()
{
	Configure_pid(1, 0, 0, 0);
	printf("1 FCM, 1");
}
void _declspec (naked) Int_1()
{
...
void Hctl_2032(BYTE Mtrnum)
{
	...
	if (Mtr_num == 1)
		...
		Hctl_xy.value;
	Hctl_oe.value = 0;     //'oe - enable output

	Hctl_sel1.value = 0; Hctl_sel2.value = 1; Poke(Temp_enc + 3, Hctl_data);       //'msb
	Hctl_sel1.value = 1; Hctl_sel2.value = 1; Poke(Temp_enc + 2, Hctl_data);  //    'byte #3
	Hctl_sel1.value = 0; Hctl_sel2.value = 0; Poke(Temp_enc + 1, Hctl_data);  //     'byte #2
	Hctl_sel1.value = 1; Hctl_sel2.value = 0; Poke(Temp_enc, Hctl_data);     //  '   'lsb
	Hctl_oe.value = 1;     // 'oe - disable output
...
{}
void Calc_trapez(BYTE mtr)
{

	if (Vel_pos[mtr] == 0)
		Vel_pos[mtr] = Vel_last[mtr];

	Rad[mtr] = DEG2RAD(Deg[mtr]);   ////    'trasforms slope degrees to radians
	Rad[mtr] = tan(Rad[mtr]); //       'calculate angle tangent
	Vel_max[mtr] = (float)Vel_pos[mtr] * 10;      //'calculate max velocity to point 1
	...
	if (Point_p1[mtr] > Diff_2[mtr])
		Point_p1[mtr] = Diff_2[mtr] / 2;

	if (Pos_final[mtr] >= Pos_encoder[mtr])
	{
		Dir_diff[mtr] = True;//       '--->
		Trapez_1[mtr] = Pos_encoder[mtr] + Point_p1[mtr];
		Trapez_2[mtr] = Pos_final[mtr] - Point_p1[mtr];
...
void Print_com(BYTE Mt, string Stringa[], long Value)
{
	//   Str_tx_1 = "(" + Str(Mt) + " " + Stringa + ", " + Str(Value) + ")"; //wlc
}
void Set_mode(BYTE mode_control)
{
	switch (mode_control)
...
long Max_minl(long I)  //       'keep value between -255 to 255
{
	if (I > 255)
		I = 255;
	else if (I < 255)
